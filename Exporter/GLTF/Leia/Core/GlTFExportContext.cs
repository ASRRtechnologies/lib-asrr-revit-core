using ASRR.Revit.Core.Exporter.GLTF.Leia.EportUtils;
using ASRR.Revit.Core.Exporter.GLTF.Leia.Export;
using ASRR.Revit.Core.Exporter.GLTF.Leia.Materials;
using ASRR.Revit.Core.Exporter.GLTF.Leia.Model;
using ASRR.Revit.Core.Exporter.GLTF.Leia.Transform;
using ASRR.Revit.Core.Exporter.GLTF.Leia.Utils;
using Autodesk.Revit.DB;
using System.Collections.Generic;
using System.Linq;
using ElementValidations = ASRR.Revit.Core.Exporter.GLTF.Leia.EportUtils.ElementValidations;
...
namespace ASRR.Revit.Core.Exporter.GLTF.Leia.Core
{
    ...
    public class GLTFExportContext : IExportContext
    {
        ...
        public GLTFExportContext(Document doc, View view, string exportPath)
        {
            currentDocument = doc;
            currentView = view;
            this.exportPath = exportPath;
            if (preferences != null) preferences.path = exportPath;
        }

        public static bool cancelation { get; set; }
        ...
        private Autodesk.Revit.DB.Transform CurrentTransform => transformStack.Peek();
        ...
        public bool Start()
        {
            preferences = new Preferences // temp
            {
                relocateTo0 = false,
                path = exportPath,
                //compression = CompressionEnum.Draco
            };
        ...
        public void Finish()
        {
            ...
            if (preferences.grids) RevitGrids.Export(currentDocument, ref nodes, ref rootNode, preferences);
        ...
        public void OnElementEnd(ElementId elementId)
        {
            ...
            foreach (var kvp in currentGeometry.Dict)
            {
                var material_key = kvp.Key.Split(UNDERSCORE)[1];
                var mat = materials.GetElement(material_key);

                var elementBinary = GLTFExportUtils.AddGeometryMeta(
                    buffers,
                    accessors,
                    bufferViews,
                    kvp.Value,
                ...
                if (preferences.normals) primitive.attributes.NORMAL = elementBinary.normalsAccessorIndex;

                if (preferences.batchId) primitive.attributes._BATCHID = elementBinary.batchIdAccessorIndex;
                ...
                if (material_key == null || material_key.Length == 0)
                {
                    continue;
                }
        ...
        public void OnMaterial(MaterialNode node)
        {
            if (preferences.materials == MaterialsEnum.materials || preferences.materials == MaterialsEnum.textures)
            {
                ...
                {
                    var materialId = node.MaterialId.ToString();
        ...
        public void OnPolymesh(PolymeshTopology polymesh)
        {
            ...
            for (var i = 0; i < pts.Count; i++) pts[i] = CurrentTransform.OfPoint(pts[i]);

            foreach (var facet in polymesh.GetFacets())
            foreach (var index in facet.GetVertices())
            {
                var vertex = pts[index];
                var vertexIndex = vertItem.AddVertexAndFlatten(
                ...
                {
                    var uv = uvs[index];
                    var flippedUV = new UV(uv.U, 1.0 - uv.V);
                    geomItem.Uvs.Add(flippedUV);
                }
            }

            if (preferences.normals) GLTFExportUtils.AddNormals(CurrentTransform, polymesh, geomItem.Normals);
        }
        ...
        public void OnRPC(RPCNode node)
        {
            ...
            foreach (var mesh in meshes)
            {
                ...
                if (triangles.Equals(0)) continue;

                if (preferences.materials == MaterialsEnum.materials || preferences.materials == MaterialsEnum.textures)
                {
                    currentMaterial =
                        MaterialUtils.GetGltfMeshMaterial(currentDocument, preferences, mesh, materials, true);
                ...
                for (var i = 0; i < triangles; i++)
                {
                    ...
                    if (triangle.Equals(null)) continue;
                    ...
                    var ptsTransformed = new List<XYZ>();
                    if (isLink)
                        ptsTransformed = pts.Select(p => linkOriginalTranformation.OfPoint(p)).ToList();
                    else
                        ptsTransformed = pts;

                    GLTFExportUtils.AddVerticesAndFaces(currentVertices.CurrentItem, currentGeometry.CurrentItem,
                        ptsTransformed);
